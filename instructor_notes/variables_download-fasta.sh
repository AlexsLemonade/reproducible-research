...
curl -O "$FASTQ_URL/$FASTQ_R1" # this approach preserves the original internet file
mv $FASTQ_R1 $FASTQ_DEST/$FASTQ_R1 # move to destination directory
...
mv $FASTQ_R1 $FASTQ_DEST/$FASTQ_R1 
...
# Curl the file (using one of several approaches)
curl -O "$FASTQ_URL/$FASTQ_R2" # this approach preserves the original internet file
mv $FASTQ_R2 $FASTQ_DEST/$FASTQ_R2  # move to destination directory

# Explore: how many lines are in the file?
gunzip -c "$FASTQ_DEST/$FASTQ_R2" | wc -l
  
# Move the file to its destination directory
mv $FASTQ_R2 $FASTQ_DEST/$FASTQ_R2
